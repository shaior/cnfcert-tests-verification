...
var _ = Describe("lifecycle-deployment-scaling", func() {
	...
	It("One deployment, one pod, one container, scale in and out", func() {
		...
		err = globalhelper.LaunchTests(lifeparameters.TnfDeploymentScalingTcName,
			globalhelper.ConvertSpecNameToFileName(CurrentGinkgoTestDescription().FullTestText))
		Expect(err).ToNot(HaveOccurred())

		By("Verify test case status in Junit and Claim reports")
		err = globalhelper.ValidateIfReportsAreValid(lifeparameters.TnfDeploymentScalingTcName,
