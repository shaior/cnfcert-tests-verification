...
var _ = Describe("lifecycle-statefulset-scaling", func() {
	...
	It("One statefulSet, one pod", func() {
		...
		err = globalhelper.LaunchTests(lifeparameters.TnfStatefulSetScalingTcName,
			globalhelper.ConvertSpecNameToFileName(CurrentGinkgoTestDescription().FullTestText))
		Expect(err).ToNot(HaveOccurred())

		By("Verify test case status in Junit and Claim reports")
		err = globalhelper.ValidateIfReportsAreValid(lifeparameters.TnfStatefulSetScalingTcName,
